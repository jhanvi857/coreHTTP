...
3. **Run**: `mvn exec:java -Dexec.mainClass=com.jhanvi857.nioflow.server.HttpServer`
4. **Environment Variables** (see README for full list):
   - `NIOFLOW_ENABLE_DB`: Set to `true` to enable PostgreSQL (default: `false`).
...
3. **Verify DB**: `docker-compose exec db psql -U postgres -d nioflow`
...
- **CORS**: Configurable via `NIOFLOW_CORS_ORIGIN` env var. Defaults to `http://localhost:3000`.
